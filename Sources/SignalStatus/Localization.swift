...
enum L10n {
    ...
    private static let simplifiedChinese: [String: String] = [
        ...
        "These Qualcomm preferences last until VOS loses power. Auto restores the captured SA/NSA masks; LTE only disables NR while preserving the current LTE band mask.": "这些 Qualcomm 设置会持续到 VOS 断电。自动模式会恢复已记录的 SA/NSA 掩码；仅 LTE 模式会关闭 NR，同时保留当前 LTE 频段掩码。",
        "Automatic defaults were not available to capture. Power-cycle VOS, then close and reopen this panel before changing radio access mode.": "未能记录自动模式默认值。请让 VOS 断电重启，然后关闭并重新打开此面板，再切换无线接入模式。",
        ...
        "This temporarily changes the Qualcomm radio access mode and band masks. LTE only disables NR and preserves the current LTE mask. Exact values are read back; a failed change triggers an automatic rollback. Selected operator settings are not changed.": "此操作会临时更改 Qualcomm 无线接入模式和频段掩码。仅 LTE 模式会关闭 NR 并保留当前 LTE 掩码。应用会读回精确值进行验证；更改失败时会自动回滚。已选运营商设置不会改变。",
