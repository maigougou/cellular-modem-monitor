...
- 支持详细、紧凑和仅图标三种菜单栏样式
- 提供英文和简体中文界面，可即时切换语言
- 提供绑定当前 modem 接口的测速，显示实时下载/上传速度及最终带宽、空闲延迟和
  响应能力结果
...
**测速**卡会调用 macOS 13 及更高版本自带的 `networkQuality`，并用 `-I` 绑定
当前活动 modem 所对应的精确接口。无法验证接口或接口索引时，应用会拒绝测速，
不会回落到默认路由。实时下载/上传值来自该接口的字节计数器；最终速度、空闲延迟
和响应能力来自 `networkQuality` 的结构化结果，并且只有结果报告同一接口时才会
被应用接受。

USB 或以太网直连时，这能把公网测速绑定到 modem 的 Mac 接口。对于
Mac → 路由器 → modem 这类路由路径，它能证明测速经过 Mac 到路由器的接口；
路由器内部的 WAN、VPN 和多 WAN 选择仍由路由器控制。测速会消耗较多流量，
请仅在可以接受该流量消耗时启动。
