...
- ZTE 偏好会跨断电持久保存。每项持久控制写入前，后端都会验证完整恢复镜像：
  ...
  **恢复自动默认值**调用专用方法
  `nwinfo_reset_band_cell_setting` 清除 LTE/NR 小区锁，并让零售固件恢复其权威的旧制式
  GW 状态。实机验证表明 MC7530CAV2.6 的该 reset 会保留原有 SA/NSA 锁频，因此后端会
  明确写回精确的 LTE、SA 与 NSA 厂商列表，再恢复并验证 `WL_AND_NSA` 自动模式。最终
  读回还会验证 reset 后的 GW 值、已清除的小区锁和精确的默认 NRDC 列表。第一次 reset
  写入前会先解析并严格校验所有可恢复的原状态；遇到响应丢失、读回失败、任务取消或
  旁带修改后，会在独立的
