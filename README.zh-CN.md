...
扫描或选择运营商、切换无线模式、勾选 NR／LTE 允许频段。ZTE 恢复已保存的锁频前频段；VOS 恢复已记录的自动默认设置。可用控制项取决于所选设备的能力。
...
- Current connection 卡片提供 Auto SA/NSA → SA only → NSA only → LTE only 快捷菜单，与完整控制面板共用确认、
  后端写入及读回验证流程
- 锁定 NR/LTE 频段，校验后写入并支持自动失败回滚；ZTE 可选项从设备读取并按物理设备
  保存，不使用硬编码型号频段表
- 后端专属的恢复操作及最终读回验证
- 每次写入前验证物理设备身份
- 在当前后端明确支持时读取 LTE 邻区测量

底栏直接提供 **复制 · 重启 · 管理页面 · 关于 · 退出**，不再收进二级菜单。
重启前会显示绑定设备的名称和管理地址供确认，VOS 通过已认证 SSH、ZTE 通过原厂
Web UBus 重启接口执行。测速或其他控制操作进行中不可重启。成功响应仅表示请求已接受，
不代表设备已完成启动；监控会自动重新连接，响应丢失时不会自动再次发送重启请求。
...
在部分 Darwin 版本上，Ookla CLI 1.2.0 只要传入 `--interface` 或 `--ip` 就无法创建 socket，但不
...
实时下载/上传值来自已验证接口的字节计数器。Ookla 机器可读结果中的 `bandwidth`
单位为字节/秒，应用会换算为比特/秒后显示。

官方 Ookla CLI 本身就按延迟、下载、上传的顺序运行，并没有额外的“顺序模式”开关
（Ookla 中的 `-s` 表示服务器 ID）。测速运行时，实时方块读取整个接口的字节
计数器，因此 TCP 确认包或接口上的其他流量可能令反方向显示少量速度；最终结果始终
采用 Ookla 自己报告的数值。

USB 或以太网直连时，这能把公网测速绑定到 modem 的 Mac 接口。对于
Mac → 路由器 → modem 这类路由路径，它能证明测速经过 Mac 到路由器的
接口；路由器内部的 WAN、VPN 和多 WAN 选择仍由路由器控制。一次测速可能消耗
...
每条载波还会独立显示**上行启用／上行禁用／上行未知**，不从 Active/Inactive 推断。
MC7530CA 对应原厂 Web UI 的 **UL Configured**：LTE SCell 取 `ltecasig` 第 5 项，
NR SCell 取 `nrca` 第 7 项（均从 1 开始计数，`1` 启用、`0` 禁用）。有效主载波沿用
原厂页面固定 Enabled 的约定，netinfo 没有独立的主载波 UL 标志。辅载波字段缺失、
数值无法识别，或其他后端尚未映射 UL 字段时显示未知；复制诊断也包含同一状态。
这表示上行配置，不表示此刻正在同时发送上行数据；现有 CC／带宽汇总不是 UL CA
数量或上行总带宽。
...
- 如果 control session 打开时 modem 已处于手动选网，`nwinfo_get_netinfo` 不会返回
  ...
  网络、恢复自动选网、改变无线模式、LTE/NR 锁频或恢复已保存频段都会在写入前安全拒绝。
...
- LTE 锁频使用 `nwinfo_set_lte_ext_band`；SA 与 NSA 都使用
  `nwinfo_set_nrbandlock`，type 分别为 `0` 与 `1`。可选频段来自已认证设备回报的
  LTE/SA/NSA 偏好，并按物理设备分别累计。设备回报的新频段会在重新加载控制后出现，
  无需修改型号白名单。当前勾选状态与累计可选项分开保存：锁定部分频段后，先前的
  选项不会从界面消失，重启应用后仍然保留。输入按适用无线模式的已保存回读记录与
  传输格式边界校验。
- **设备回报的已保存频段**不是 DMS 硬件能力查询，也不是附近网络扫描，不能证明
  所列频段能实际驻网、参与聚合，或能通过该固件的某个 Web setter 写入。若首次连接时
  设备已锁频，已知列表可能不完整。之后的真实设备回读可以扩展列表；应用不会推测
  缺失频段，也不会借用其他设备的记录。
- ZTE 偏好会跨断电持久保存。每项控制写入前，后端都会验证可恢复的操作前状态。
  命令成功还必须证明请求字段之外的所有持久字段保持不变；固件旁带修改会按失败处理。
- 锁频与**恢复已保存频段**采用针对性恢复，不使用厂商全局频段/小区 reset。
  响应丢失、验证失败或任务取消后，独立的未取消任务会恢复已改变且已知可恢复的字段，
  并验证完整结果。若小区锁或 NRDC 出现无法恢复的变化，会明确报告，不会试探性地
  全局 reset。零售 schema 没有 NRDC setter，但合法的非默认 NRDC 列表只要保持不变，
  就不会阻止仅频段操作。格式不合法的旧制式 GW 或小区锁状态仍会在写入前阻止操作，
  因为无法安全恢复。
  **恢复已保存频段**只恢复当前物理设备已保存的锁频前 LTE、SA 和 NSA 偏好；不调用
  厂商全局频段/小区 reset，不写回原厂或型号频段表，也不切换无线模式或运营商选择。
  旧制式 GW 偏好、NRDC 和 LTE/NR 小区锁必须保持不变；精确回读会同时验证恢复字段与
  保留字段。这不是恢复出厂设置。第一次写入前会先解析并严格校验所有可恢复的原状态。
- 运营商选择和无线模式切换保留原有的全状态恢复路径。由于该路径可能通过
  `nwinfo_reset_band_cell_setting` 及零售 setter 重建此前的
  GW/LTE/SA/NSA/小区锁/选网值，操作前仍要求 NRDC 为精确的已验证默认列表。
  某一步响应丢失不会阻止后续恢复步骤，最终以精确权威读回判定成功。
  这不是恢复出厂设置；上述控制均不会读取或修改 APN profile。
...
按物理设备的 SHA-256 指纹区分，不按管理 IP 区分。文件包含频段集合与时间戳，
不含密码或原始设备序列号；采用原子写入，文件权限仅允许当前用户访问（`0600`）。

首次成功读取会记录已知可选项。第一次锁频前，应用另行保存当时精确的 LTE/SA/NSA
tuple 作为恢复点。后续锁频与回读可以扩展可选项，但不会覆盖该锁频前恢复点。
**恢复已保存频段**会还原此恢复点并验证结果；下一轮锁频会捕获新的恢复点。
累计可选项本身不是恢复目标。
应用尚未保存锁频前恢复点时，恢复按钮不可用；它不能撤销此前在其他地方做的锁频，
也不能推算未知的更宽频段能力。

如需独立备份，请先退出 CMM，再将 `band-baselines.json` 复制到安全位置，尤其是在
替换或删除应用数据之前。若本地文件损坏，请保留原文件供排查，在 CMM 关闭时恢复
已知正常的备份，然后在同一物理设备上重新打开“网络与无线控制”。损坏或未知版本的
文件不会被静默覆盖。只恢复本地文件不会改变设备：还原设备频段仍需在应用中明确
确认并通过权威读回验证。若没有匹配备份，设备又已经限频，重新连接或重启不能找回
从未读到的更宽频段列表；应使用该设备已知的恢复记录，而不是猜测原厂默认值。
...
拒写；设备回报频段的累计、按物理设备持久保存的基线及仅频段恢复；凭据策略与不含秘密的偏好数据；后端 registry/coordinator
选择；畸形响应；
...
- ZTE 可选频段是累计的设备回报偏好，不是完整硬件能力表。首次读取时已锁频的设备
  可能只有不完整列表；迁移到另一台 Mac 时请一并保留已保存频段文件。
