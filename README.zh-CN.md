...
两种工具都会绑定当前活动 modem 对应的精确接口。无法验证接口或接口索引时，应用
会拒绝测速，不会回落到默认路由；两个按钮也会互锁，避免两种测速同时争抢带宽。
实时下载/上传值来自该接口的字节计数器。最终 `networkQuality` 结果必须报告同一
接口；最终 Ookla JSON 必须同时报告同一接口和已冻结的内部 IP，结果才会被接受。
Ookla 机器可读结果中的 `bandwidth` 单位为字节/秒，应用会换算为比特/秒后统一显示。
